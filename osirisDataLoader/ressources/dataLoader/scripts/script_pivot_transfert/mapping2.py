...
class mapping_osiris:
    ...
    def transfert_concept_from_pivot(self, i2b2_var, patient, i2b2_concept_ref, visit):
        dic_info = {}
        ...
        try:
            dic_info['tvalue'] = self.obj_pivot.list_key_ref[var_ref]['listPatient'][patient][
                re.match('.*_', var_ref).group(0) + i2b2_var]
        except:
            dic_info['tvalue'] = None
        dic_info['sourcesystem_cd'] = self.source
        concept_moda = self.searchModalityID(i2b2_var, dic_info['tvalue'])
        if concept_moda != None: dic_info['concept_cd'] = concept_moda
        else: dic_info['concept_cd'] = i2b2_concept_ref
    ...
    def transfert_patient_from_pivot(self, i2b2_var, patient, i2b2_concept_ref):
        dic_info = {}
        var_ref = self.find_refvar_in_pivot(i2b2_var[len(re.match('[a-zA-Z0-9]*\|', i2b2_var).group(0)):len(re.match('.*\|', i2b2_var).group(0))-1])
        dic_info['date'] = self.default_date
        dic_info['table'] = 'concept_dimension'
        ...
        try:
            dic_info['tvalue'] = self.obj_pivot.list_key_ref[var_ref]['listPatient'][patient][
                'Patient_' + i2b2_var[
                len(re.match('[a-zA-Z0-9]*\|', i2b2_var).group(0)):len(re.match('.*\|', i2b2_var).group(0)) - 1]]
        except:
            dic_info['tvalue'] = None
        dic_info['sourcesystem_cd'] = self.source
    ...
    def transfert_visit_from_pivot(self, i2b2_var, patient, visit, i2b2_concept_ref):
        ...
        concept_moda = self.searchModalityID('tumorPathologyEvent', i2b2_var, dic_info['tvalue'])
    ...
    def transfert_concept_old_from_pivot(self, i2b2_var, patient, instance, i2b2_type, i2b2_basecode):
        dic_info = {}
        var_ref = self.find_refvar_in_pivot(i2b2_var)
        dic_info['patient_num'] = self.obj_pivot.dic_patient[patient]
        dic_info['encounter_num'] = self.obj_pivot.list_key_ref[var_ref]['listPatient'][patient][instance][
            'TumorPathologyEvent_Ref']
        dic_info['var'] = i2b2_var
        if i2b2_type == 'N':
            try:
                dic_info['nvalue'] = float(self.obj_pivot.list_key_ref[var_ref]['listPatient'][patient][instance][
                                               self.find_matchvar_in_pivot(i2b2_var)])
                dic_info['tvalue'] = 'E'
            except:
                dic_info['nvalue'] = None
                dic_info['tvalue'] = None
            ...
            dic_info['nvalue'] = None
            try:
                dic_info['value'] = self.obj_pivot.list_key_ref[var_ref]['listPatient'][patient][instance][
                    self.find_matchvar_in_pivot(i2b2_var)]
            except:
                dic_info['tvalue'] = None
